...
* **ST-GCN回顧**
    ...
    5. GCN塊F包含兩個組件：一個空間模塊G和一個時間模塊T。給定一個骨架特徵$X ∈ R^{T×V×C}$，G首先執行通道膨脹和重塑以獲得4D張量$X' ∈ R^{T×K×V×C}$，然後使用一組係數矩陣A進行關節間的空間建模：$G(X)_{lij}=\displaystyle\sum_{k=1}^{k=K}\displaystyle\sum_{v=1}^{v=V}A_{kvi}X'_{lkvj}, G(X)∈R^{T×V×C}$
    
    6. G的輸出進一步由T進行處理以學習時間特徵。F的計算可以總結為：$F(X)=T(G(X,A))+X$
...
* **DG-GCN：從零開始的動態空間建模**
    ...
    * **係數矩陣A~i~：** 每個係數矩陣Ai包含三個可學習的組件。
        ...
        4. DA~i~：$DA_i=Softmax(X_aX_b,dim=0)$
        5. CA~i~：$CA_i ∈ R^{C×V×V}$，通過將Tanh激活應用於X~a~和X~b~之間的兩兩差異來獲得CA~i~。
        6. 係數矩陣使用三個組件的加權和：$A_i=PA_i+α \times DA_i+β \times CA_i$
        7. $α$，$β$是兩個可學習的參數。
